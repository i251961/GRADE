/* =====================================================
   ...
   objeto interno organizado por área:
   {
     ...
     gradeSemanal: { instancias: [] }, // cartões da Grade Semanal (horários de aula)
...
function _estruturaPadrao() {
  return {
    ...
    simulacoes: [],
    configuracoes: {},
    gradeSemanal: { instancias: [] },
    disciplinasCustom: [], // disciplinas adicionadas manualmente pelo usuário
...
/** Grade Semanal: cartões de horário (dia/hora) montados pelo usuário. */
function salvarGradeSemanalStorage(gradeSemanal) {
  const dados = _obterDados();
  dados.gradeSemanal = gradeSemanal || { instancias: [] };
  _persistir();
}

function carregarGradeSemanalStorage() {
  const dados = _obterDados();
  // dados salvos antes dessa funcionalidade existir não têm gradeSemanal — cai no padrão
  if (!dados.gradeSemanal || !Array.isArray(dados.gradeSemanal.instancias)) {
    return { instancias: [] };
  }
  return dados.gradeSemanal;
}

/** Disciplinas customizadas (adicionadas manualmente pelo usuário via "+ Nova disciplina"). */
function salvarDisciplinasCustomStorage(lista) {
  const dados = _obterDados();
  dados.disciplinasCustom = lista || [];
  _persistir();
}

function carregarDisciplinasCustomStorage() {
  const dados = _obterDados();
  // dados salvos antes dessa funcionalidade existir não têm esse campo — cai no padrão
  if (!Array.isArray(dados.disciplinasCustom)) return [];
  return dados.disciplinasCustom;
}
