...
const GradeSemanal = (function () {
  ...
  // arrastoInfo: { id, duracao, grabOffsetY } — grabOffsetY é a distância (em px)
  // entre o topo do cartão e o ponto exato onde o usuário "pegou" o cartão
  // (mouse ou dedo). É o que permite que o cartão não "salte" para debaixo do
  // cursor: o topo do cartão-fantasma continua acompanhando o mesmo ponto em
  // que foi agarrado, exatamente como no Trello/Google Calendar/Notion.
  let arrastoInfo = null;
  let previewEl = null;

  // Posição (dia + slot) indicada pelo preview transparente no momento atual
  // do arraste. É ESSA posição — e não a do dedo/cursor no instante do drop —
  // que é usada para efetivamente alocar o cartão ao soltar.
  let previewDiaAtual = null;
  let previewSlotAtual = null;
  ...
  function limparPreview() {
    ...
    previewDiaAtual = null;
    previewSlotAtual = null;
  }

  /**
   * Calcula o slot onde o TOPO do cartão ficaria (considerando o ponto onde
   * ele foi agarrado) e desenha o preview transparente nessa posição.
   * `topoDesejadoPx` já vem descontado do grabOffsetY — é a coordenada de
   * página (viewport) onde o topo do cartão ficaria.
   */
  function atualizarPreviewGrade(colEl, diaIdx, topoDesejadoPx) {
    if (!arrastoInfo) return;
    const rect = colEl.getBoundingClientRect();
    const duracaoSlots = Math.max(1, Math.round(arrastoInfo.duracao / SLOT_MIN));
    const maxSlot = Math.max(0, TOTAL_SLOTS - duracaoSlots);
    const slot = Math.max(0, Math.min(maxSlot, Math.round((topoDesejadoPx - rect.top) / SLOT_PX)));
    ...
    previewDiaAtual = diaIdx;
    previewSlotAtual = slot;
  }
  ...
  function ativarArrastoDesktop(cardEl, instId) {
    ...
    cardEl.addEventListener("dragstart", (e) => {
      ...
      arrastoInfo = {
        id: instId,
        duracao: inst ? inst.duracao : DURACAO_PADRAO_MIN,
  ...
  function ativarColunaAlvoDesktop(colEl, diaIdx) {
    colEl.addEventListener("dragover", (e) => {
      ...
      if (!arrastoInfo) return;
      const topoDesejadoPx = e.clientY - (arrastoInfo.grabOffsetY || 0);
      atualizarPreviewGrade(colEl, diaIdx, topoDesejadoPx);
    });
    ...
    colEl.addEventListener("drop", (e) => {
      ...
      if (!id) return;
      // Usa a posição indicada pelo preview — não a posição do cursor no
      // instante do drop — garantindo que o cartão pouse exatamente onde
      // o preview transparente mostrou.
      if (previewDiaAtual !== null && previewSlotAtual !== null) {
        alocar(id, previewDiaAtual, HORA_INICIO_MIN + previewSlotAtual * SLOT_MIN);
      } else {
        const rect = colEl.getBoundingClientRect();
  ...
  function ativarArrastoTouch(cardEl, instId) {
    ...
    function iniciarArraste() {
      ...
      const inst = instancias.find((i) => i.id === instId);
      // grabOffsetY: distância do topo do cartão original até o ponto exato
      // onde o dedo tocou — preserva esse ponto durante todo o arraste.
      arrastoInfo = {
        id: instId,
        duracao: inst ? inst.duracao : DURACAO_PADRAO_MIN,
        grabOffsetY: startY - originTop,
      };
    ...
    cardEl.addEventListener(
      ...
      (e) => {
        ...
        e.preventDefault();
        // Topo do cartão-fantasma (mesma coordenada usada para posicioná-lo
        // visualmente), preservando o ponto onde o dedo agarrou o cartão.
        const topoFantasma = originTop + dy;
        ghost.style.left = originLeft + dx + "px";
        ghost.style.top = topoFantasma + "px";

        limparPreview();
        const alvo = alvoSobPonto(t.clientX, t.clientY);
        if (alvo && alvo.tipo === "grade") {
          alvo.col.classList.add("drag-over");
          atualizarPreviewGrade(alvo.col, parseInt(alvo.col.dataset.dia, 10), topoFantasma);
    ...
    function finalizar(e) {
      ...
      if (arrastando) {
        ...
        const alvo = alvoSobPonto(t.clientX, t.clientY);
        // Usa a posição indicada pelo preview transparente — não a posição
        // do dedo ao soltar — igual ao Google Calendar/Trello/Notion.
        if (alvo && alvo.tipo === "grade" && previewDiaAtual !== null && previewSlotAtual !== null) {
          alocar(instId, previewDiaAtual, HORA_INICIO_MIN + previewSlotAtual * SLOT_MIN);
        } else if (alvo && alvo.tipo === "pool") {
          desalocar(instId);
        }
      }
