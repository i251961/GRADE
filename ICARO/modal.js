...
const Modal = (function () {
  ...
  function init(config) {
    ...
    document.getElementById("modal-excluir").addEventListener("click", () => {
      if (!codigoAtual) return;
      const confirmar = confirm(`Excluir a disciplina ${codigoAtual}? Essa ação não pode ser desfeita pelo undo.`);
      if (!confirmar) return;
      deps.onRemover(codigoAtual);
      fechar();
    });
  }
  ...
  function abrir(codigo) {
    ...
    document.getElementById("modal-excluir").classList.toggle("show", !!d.custom);
