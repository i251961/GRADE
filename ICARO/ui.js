...
const UI = (function () {
  ...
  function init(config) {
    ...
    DragDrop.ativarAutoScrollBoard(boardEl);
  ...
  function criarCard(estado, d) {
    ...
    } else if (d.restricaoOferecimento) {
      const tag = document.createElement("span");
      tag.className = "card-restricao-tag";
      tag.title = rotuloRestricaoOferecimento(d);
      tag.textContent = d.restricaoOferecimento === "par" ? "só pares" : "só ímpares";
      bottom.appendChild(tag);
    }
  ...
  function atualizarDashboard(estado) {
    const total = totalDisciplinasCurso();
    ...
    const totalCreditos = totalCreditosCurso();
    const creditosRestantes = totalCreditos - creditosConcluidos;
    const percentual = totalCreditos > 0
      ? Math.round((creditosConcluidos / totalCreditos) * 100)
      : 0;
